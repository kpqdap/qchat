...
export const NavBar: React.FC = () => {
  ...
  const homeLink: LinkItem = { name: 'Home', href: '/', icon: HomeIcon };

  const settingsLink: LinkItem | undefined = session ? { name: 'Settings', href: '/settings', icon: UserCog } : undefined;

  const links: LinkItem[] = [homeLink];
  if (settingsLink) {
    ...
    <nav aria-label="Main navigation" className="bg-backgroundShade border-b-4 border-accent">
      <div className="container mx-auto hidden md:block">
        <div dir="ltr" className="grid grid-cols-12 gap-2 items-center">
          ...
          <div className="col-span-8 flex justify-end min:h-[40px]">
            <ThemeSwitch />
          </div>
        </div>
      </div>
