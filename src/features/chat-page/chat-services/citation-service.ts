...
export const CreateCitation = async (model: ChatCitationModel): Promise<ServerActionResponse<ChatCitationModel>> => {
  try {
    const { resource } = await HistoryContainer().items.create<ChatCitationModel>(model)
...
export const FindCitationByID = async (id: string): Promise<ServerActionResponse<ChatCitationModel>> => {
  try {
    const querySpec: SqlQuerySpec = {
      query: "SELECT * FROM root r WHERE r.type=@type AND r.id=@id AND r.userId=@userId ",
      parameters: [
        {
          name: "@type",
    ...
    const { resources } = await HistoryContainer().items.query<ChatCitationModel>(querySpec).fetchAll()
...
export const FormatCitations = (citation: DocumentSearchResponse[]): DocumentSearchResponse[] => {
